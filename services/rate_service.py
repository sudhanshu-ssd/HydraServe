...
async def get_limits(model:str,project_id:int,db:AsyncSession):  
    results  = await db.execute(select(models.Models).options(selectinload(models.Models.provider)).where(models.Models.model_name == model))
    ...
    provider_name = my_model.provider.name
    ...
    print("got limits")
    return g_rpm,g_rpd,g_tpm,g_tpd,rpm,rpd,tpm,tpd,model_id,provider_name
...
if redis.call('ZCOUNT', KEYS[3], minute_ago, now) >= tonumber(ARGV[10]) then return {0, "model_rpm"}   end -- ARGV[10] was wrong in Claude's version
...
async def is_allowed(redis_client : aioredis.Redis , model: str, project_id: int, db: AsyncSession):
    print("entered the rate limiting")
    g_rpm, g_rpd, g_tpm, g_tpd, rpm, rpd, tpm, tpd, model_id,provider_name = await get_limits(model, project_id, db)
    ...
    return allowed, reason, member_id, model_id,provider_name


async def correct_tokens(redis_client: aioredis.Redis, project_id: int, model_id: int, member_id: str, actual_tokens: int):
    print("Entering correct tokens")
    old_member = f"{member_id}:{settings.estimated_tokens}"
